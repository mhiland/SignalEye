...
def parse_encryption_info(cell_data):
    ...
    if "Encryption key:on" in cell_data:
        ...
        if wpa3_info:
            ...
            group_cipher = re.search(r'Group Cipher : (\w+)', wpa3_section).group(1)
            pairwise_ciphers_match = re.search(r'Pairwise Ciphers \(\d+\) : ([\w\s]+?)(?=Authentication Suites)', wpa3_section, re.DOTALL)
            pairwise_ciphers = re.sub(r'\s+', ' ', pairwise_ciphers_match.group(1)).strip() if pairwise_ciphers_match else None
            auth_suites = re.search(r'Authentication Suites \(\d+\) : (\w+)', wpa3_section).group(1)
        ...
        if wpa2_info:
            ...
            pairwise_ciphers_match = re.search(r'Pairwise Ciphers \(\d+\) : ([\w\s]+?)(?=Authentication Suites)', wpa2_section, re.DOTALL)
            pairwise_ciphers = re.sub(r'\s+', ' ', pairwise_ciphers_match.group(1)).strip() if pairwise_ciphers_match else None
            encryption_info["WPA2"] = {
                "Group Cipher": re.search(r'Group Cipher : (\w+)', wpa2_section).group(1),
                "Pairwise Ciphers": pairwise_ciphers,
                "Authentication Suites": re.search(r'Authentication Suites \(\d+\) : (\w+)', wpa2_section).group(1)
            }
        ...
        if wpa_info:
            ...
            pairwise_ciphers_match = re.search(r'Pairwise Ciphers \(\d+\) : ([\w\s]+?)(?=Authentication Suites)', wpa_section, re.DOTALL)
            pairwise_ciphers = re.sub(r'\s+', ' ', pairwise_ciphers_match.group(1)).strip() if pairwise_ciphers_match else None
            encryption_info["WPA"] = {
                "Group Cipher": re.search(r'Group Cipher : (\w+)', wpa_section).group(1),
                "Pairwise Ciphers": pairwise_ciphers,
                "Authentication Suites": re.search(r'Authentication Suites \(\d+\) : (\w+)', wpa_section).group(1)
            }

        # Check for WEP encryption
        if "Encryption key:on" in cell_data and not (wpa3_info or wpa2_info or wpa_info):
...
def parse_cell_information(cell_data):
    ...
    try:
        ...
        info["Frequency"] = re.search(r'Frequency:([\d\.]+ GHz)', cell_data).group(1).split(' ')[0]
        info["Channel"] = re.search(r'Channel:(\d+)', cell_data).group(1)
        info["Quality"] = re.search(r'Quality=([\d/]+)', cell_data).group(1).split('/')[0]
        info["Signal Level"] = re.search(r'Signal level=([\-\d]+ dBm)', cell_data).group(1).split(' ')[0]
